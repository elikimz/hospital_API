...
#     # Allow admin to delete any appointment
...
#         .filter(model.Appointment.patient_id == current_user.id)
#         .all()
#     )
...
#     db_appointment = (
#         db.query(model.Appointment).filter(model.Appointment.id == id).first()
#     )
...
@router.delete("/{id}", response_model=schema.AppointmentResponseWithPatient)
def delete_appointment(
    id: int,
    db: Session = Depends(database.get_db),
    current_user: model.User = Depends(get_current_user),
...
@router.get("/my-appointments", response_model=List[schema.AppointmentResponseWithPatient])
def get_patient_appointments(
    db: Session = Depends(database.get_db),
    current_user: model.User = Depends(get_current_user),
):
    ...
    appointments = (
        db.query(model.Appointment)
        .filter(model.Appointment.patient_id == current_user.id)
        .options(joinedload(model.Appointment.patient).joinedload(model.Patient.user))
        .all()
    )
...
@router.put("/{id}", response_model=schema.AppointmentResponseWithPatient)
def update_appointment(
    id: int,
    appointment_update: schema.AppointmentUpdate,
    db: Session = Depends(database.get_db),
...
@router.get("/", response_model=List[schema.AppointmentResponseWithPatient])
def get_all_appointments(
    db: Session = Depends(database.get_db),
    current_user: model.User = Depends(get_current_user),
):
...
@router.post("/", response_model=schema.AppointmentResponseWithPatient)
def create_appointment(
    appointment: schema.AppointmentCreate,
    db: Session = Depends(database.get_db),
    current_user: model.User = Depends(get_current_user),
...
@router.get("/{id}", response_model=schema.AppointmentResponseWithPatient)
def get_appointment(id: int, db: Session = Depends(database.get_db)):
    appointment = (
        db.query(model.Appointment)
        .options(joinedload(model.Appointment.patient).joinedload(model.Patient.user))
        .filter(model.Appointment.id == id)
        .first()
    )
    if not appointment:
        raise HTTPException(status_code=404, detail="Appointment not found")

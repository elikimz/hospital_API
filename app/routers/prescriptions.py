...
@router.get("/my-prescriptions", response_model=List[PrescriptionOut])
async def get_my_prescriptions(
    ...
    if current_user.role != model.UserRole.PATIENT:
        raise HTTPException(
            status_code=403, detail="Only patients can view their prescriptions."
        )
    ...
    prescriptions = (
        ...
        .filter(model.Prescription.patient_id == current_user.id)
        .all()
    )
